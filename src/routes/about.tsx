      ...
      <section className="max-w-5xl mx-auto px-4 py-14">
        ...
        <div className="grid md:grid-cols-2 gap-10 mt-8 items-start">
          ...
          <div className="flex justify-center">
            <img src={PORTRAIT} alt="Founder" className="max-h-[550px] object-contain" />
          </div>
        </div>

        <div className="grid md:grid-cols-3 gap-6 mt-16">
          {cards.map((c, i) => (
            <div key={i} className="border rounded-lg overflow-hidden shadow-sm bg-muted">
              <img src={c.img} alt="" className="w-full h-48 object-cover" />
              <p className="p-5 text-sm text-muted-foreground">{c.text}</p>
            </div>
          ))}
        </div>
      </section>
